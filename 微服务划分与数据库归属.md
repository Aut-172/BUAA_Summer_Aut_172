...
| `frontend/` | Vite/React 前端，运行时调用 `/api/**`。 | 不计入业务微服务数量，经 API Gateway 路由到各服务；Playwright E2E 仍使用 mock API 稳定覆盖页面流程。 |
...
| 商家商品服务 | `GET /api/merchants`、`GET /api/merchants/{id}`、`GET /api/products/{id}`、`GET /api/categories`、`GET/PUT /api/merchant/profile`、`GET/POST/PUT/DELETE /api/merchant/products/**`、`GET /api/merchant/dashboard` | `MerchantController`、商家后台相关 Controller | 公开目录浏览、商家资料、商品管理和经营看板均由 `merchant-service` 承接。 |
...
| 差异项 | 当前状态 | 收尾建议 |
| --- | --- | --- |
| 商家后台资料和商品管理 | 前端 `frontend/src/utils/api.js` 调用了 `/api/merchant/dashboard`、`GET/PUT /api/merchant/profile`、`GET/POST/PUT/DELETE /api/merchant/products/**`，后端已由 `merchant-service` 承接，Gateway 已路由。 | 交付前重点做真实 Gateway 多角色冒烟，确认前端页面与真实接口字段一致。 |
| 管理员商家管理 | 前端和 Gateway 记录了 `/api/admin/merchants/**`，后端已由 `merchant-service` 承接。 | 与管理员用户、骑手管理一并纳入真实 Gateway 冒烟。 |
...
| 搜索/推荐/仪表盘 | 商家商品服务负责商家/商品搜索推荐和商家看板；骑手看板由履约服务提供；管理员看板由管理端按主体列表和订单列表聚合展示。 | 聚合型看板优先由 Owner 服务提供统计片段或后续 BFF 聚合，不跨库联表。 |
